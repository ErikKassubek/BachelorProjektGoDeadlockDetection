...
Graph, wobei $L_i$ die Menge aller in der Routine vorkommenden Locks ist, und 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/trees_example.png}
    \caption{Graphische Darstellung der Lock-Bäume aus dem Beispiel. Die Knoten 
    entsprechen dabei Locks. In Rot ist der Lock-Baum von $R_1$, in grün 
    der Lock-Baum für $R_2$ gegeben. Durchgezogene Pfeile stellen die Dependencies
    dar, gestrichelte Pfeile werden zur Verdeutlichung des Zyklus verwendet.}
\end{figure}
Die Kanten entsprechen dabei immer Teilen der Dependencies, wobei es immer 
eine Kante von \texttt{mu} zu jedem der Locks in \texttt{hs} gibt. Verbindet man nun 
gleiche Locks zwischen den einzelnen Bäumen (im Bild mit gestrichelten Pfeilen),
besitzt der entstehende Graph einen Zyklus zwischen $mu_2$ und $mu_3$. 
