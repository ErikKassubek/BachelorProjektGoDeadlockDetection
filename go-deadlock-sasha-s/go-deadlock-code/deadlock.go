...
func lock(lockFn func(), ptr interface{}) {
	// if detection is not requested, just lock the variable
	if Opts.Disable {
		lockFn() // lock sync.Mutex
		return
	}
	stack := callers(1) // get the stack of called functions
	preLock(stack, ptr)
...
// main function to detect potential deadlocks
func (l *lockOrder) preLock(stack []uintptr, p interface{}) {
	if Opts.DisableLockOrderDetection {
		return
	}
	gid := goid.Get() // get id of current go routine
	l.mu.Lock()       // lock mutex of lock order
	for b, bs := range l.cur {
		if b == p { // it the current mutex is already in l.cur
			if bs.gid == gid { // if the mutex was locked from the same go routine
				// print out message for detected deadlock caused by recursive locking
				Opts.mu.Lock()
		...
		if bs.gid != gid { // Ignore locks from different go routines
			continue
		}
